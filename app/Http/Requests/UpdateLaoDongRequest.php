...
class UpdateLaoDongRequest extends FormRequest
{
    ...
    {
        $laoDong = $this->route('hoSo') ?? $this->route('ho_so') ?? $this->route('lao_dong') ?? $this->route('laoDong') ?? $this->route('ho-so');
        $id = is_object($laoDong) ? $laoDong->id : $laoDong;

        return [
            'nhan_khau_id' => ['required', 'integer', 'exists:nhan_khau,id', Rule::unique('lao_dong', 'nhan_khau_id')->ignore($id)],
            ...
            'ly_do_thay_doi' => 'nullable|string|max:500',
