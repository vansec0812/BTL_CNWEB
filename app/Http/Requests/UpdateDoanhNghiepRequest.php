...
class UpdateDoanhNghiepRequest extends FormRequest
{
    ...
    {
        $doanhNghiep = $this->route('doanh_nghiep') ?? $this->route('doanhNghiep');
        $id = is_object($doanhNghiep) ? $doanhNghiep->id : $doanhNghiep;

        return [
            'ten_co_so' => 'required|string|max:255',
            'ma_so_thue' => ['nullable', 'string', 'max:50', Rule::unique('doanh_nghiep_ho_kinh_doanh', 'ma_so_thue')->ignore($id)],
