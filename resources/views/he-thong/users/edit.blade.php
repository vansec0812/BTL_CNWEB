...
@section('page_title', 'Chỉnh sửa thông tin cán bộ')
        ...
        <form action="{{ route('users.update', $user->id) }}" method="POST" novalidate>
            ...
            <div class="row g-3">
                ...
                <div class="col-12 border-bottom pb-2">
                    <h6 class="fw-bold text-primary mb-0"><i class="bi bi-shield-lock me-1"></i>Thông tin đăng nhập & Vai trò</h6>
                </div>

                <div class="col-md-6">
                    <label for="name" class="form-label small fw-semibold text-secondary">Họ và tên cán bộ <span class="text-danger">*</span></label>
                    <input type="text" name="name" id="name" class="form-control form-control-sm @error('name') is-invalid @enderror" value="{{ old('name', $user->name) }}" placeholder="Ví dụ: Nguyễn Văn A" style="border-radius: 8px;" required>
                    @error('name')
                ...
                <div class="col-md-6">
                    ...
                    <input type="email" name="email" id="email" class="form-control form-control-sm @error('email') is-invalid @enderror" value="{{ old('email', $user->email) }}" placeholder="Viết thường, ví dụ: nva@ubnd-xa.vn" style="border-radius: 8px;" required>
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    <label for="password" class="form-label small fw-semibold text-secondary">Mật khẩu mới (Để trống nếu không đổi)</label>
                    <input type="password" name="password" id="password" class="form-control form-control-sm @error('password') is-invalid @enderror" placeholder="Tối thiểu 6 ký tự..." style="border-radius: 8px;">
                    @error('password')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 border-bottom pb-2 mt-4">
                    <h6 class="fw-bold text-primary mb-0"><i class="bi bi-file-earmark-person me-1"></i>Hồ sơ cá nhân & Chức danh</h6>
                </div>

                <div class="col-md-4">
                    <label for="so_cccd" class="form-label small fw-semibold text-secondary">Số CCCD (12 số)</label>
                    <input type="text" name="so_cccd" id="so_cccd" class="form-control form-control-sm @error('so_cccd') is-invalid @enderror" value="{{ old('so_cccd', $user->so_cccd) }}" placeholder="Số định danh cá nhân" style="border-radius: 8px;">
                    @error('so_cccd')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="date" name="ngay_sinh" id="ngay_sinh" class="form-control form-control-sm" value="{{ old('ngay_sinh', $user->ngay_sinh ? $user->ngay_sinh->format('Y-m-d') : '') }}" style="border-radius: 8px;">
                </div>

                <div class="col-md-6">
                    <label for="so_dien_thoai" class="form-label small fw-semibold text-secondary">Số điện thoại liên hệ</label>
                    <input type="text" name="so_dien_thoai" id="so_dien_thoai" class="form-control form-control-sm" value="{{ old('so_dien_thoai', $user->so_dien_thoai) }}" placeholder="Ví dụ: 0912345678" style="border-radius: 8px;">
                </div>

                <div class="col-md-6">
                    <label for="chuc_vu" class="form-label small fw-semibold text-secondary">Chức danh hành chính (Chức vụ)</label>
                    <input type="text" name="chuc_vu" id="chuc_vu" class="form-control form-control-sm" value="{{ old('chuc_vu', $user->chuc_vu) }}" placeholder="Ví dụ: Cán bộ địa chính, Cán bộ quân sự..." style="border-radius: 8px;">
                </div>
                ...
                <div class="col-12 border-top pt-3 mt-3">
                    <span class="small fw-semibold text-primary d-block mb-2"><i class="bi bi-geo-alt me-1"></i>Quê quán (Tỉnh / Huyện / Xã)</span>
                    <div class="row g-2">
                        <div class="col-md-4">
                    ...
                    <input type="hidden" name="que_quan" id="que_quan_hidden" value="{{ old('que_quan', $user->que_quan) }}">
                </div>

                <div class="col-12 border-top pt-3 mt-3">
                    <span class="small fw-semibold text-primary d-block mb-2"><i class="bi bi-house-door me-1"></i>Địa chỉ nơi ở hiện tại</span>
                    <div class="row g-2">
                        <div class="col-md-3">
                    ...
                    <input type="hidden" name="dia_chi" id="dia_chi_hidden" value="{{ old('dia_chi', $user->dia_chi) }}">
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const initialQueQuan = @json(old('que_quan', $user->que_quan ?? ''));
    const initialDiaChi = @json(old('dia_chi', $user->dia_chi ?? ''));
